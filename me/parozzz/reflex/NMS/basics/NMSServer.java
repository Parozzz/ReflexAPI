...
public class NMSServer implements NMSWrapper
{
    ...
    static
    {
        Class<?> craftWorldClass = ReflectionUtil.getCraftbukkitClass("CraftServer");
        
        getHandle = ReflectionUtil.getMethod(craftWorldClass, "getHandle");
        sendAll = Stream.of(ReflectionUtil.getNMSClass("DedicatedPlayerList").getMethods())
                .filter(m -> m.getName().contains("sendAll"))
